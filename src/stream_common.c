...
int decodeAllHeaders(int respac, struct streamstate *s, enum streamtype type) {
	...
	if (res != TH_ENOTFORMAT) {
	    ...
	    // inserer votre code ici !!
            pthread_t draw2sdl_tid;
            int creation_error_draw2sdl = pthread_create(&draw2sdl_tid, NULL, draw2SDL, &s->serial);
            if (creation_error_draw2sdl){
                perror("\n Error during creation of draw2SDL thread");
            } else {printf("\nDraw2SDL successfully created");}
